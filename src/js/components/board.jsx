...
} from "../factories/board-shapes-factory";
...
export default class Board extends React.Component {
    getBackgroundForSquare(square) {
        if (this.props.squareSelectionState.isSquareSelected(square)) {
            ...
            this.props.ambiguousMoveState.isAmbiguousDestinationSelected()
        ) {
            if (
                this.props.ambiguousMoveState.getSelectedAmbiguousDestination() ==
                square
            ) {
                return this.props.displayState.getPossibleMoveBackground();
            } else if (
                this.props.ambiguousMoveState.isDisambiguatingCaptureForSelectedSquare(
                    square
                )
            ) {
            ...
            this.props.squareSelectionState.isSquarePossibleCapture(square)
        ) {
            return this.props.displayState.getPossibleCaptureBackground();
        } else if (
            this.props.squareSelectionState.isSquarePossibleMove(square)
        ) {
            return this.props.displayState.getPossibleMoveBackground();
        }
    ...
    getClassForSquare(square) {
        ...
        return `square ${
            this.props.gameState.hasPieceOnSquare(square) ? "piece-square" : ""
        }`;
    }
    ...
    render() {
        ...
        return (
            <div
                id="board-block"
                className="content-block mui-col-md-7 mui-col-sm-12"
            >
                <table id="board" className="piece-grid">
                    <tbody>
                        {boardShape.map((row, rowIndex) => (
                            <tr key={rowIndex}>
                                {row.map((square) => (
                                    <td
                                        key={squareIndex++}
                                        className={this.getClassForSquare(
                                            square
                                        )}
                                        style={this.getStyleForSquare(square)}
                                        onClick={() =>
                                            this.props.clickHandler(square)
                                        }
                                    >
                                        <div>
                                            {this.getPieceImageElementForSquare(
                                                square
                                            )}
                                        </div>
                                    </td>
